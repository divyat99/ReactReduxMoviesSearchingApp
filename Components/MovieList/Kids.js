...
const Kids = () => {
 ...
 useEffect(() => {
  const fetchDataFromAPI = async () => {
    try {
      const kids = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=cfe422613b250f702980a3bbf9e90716&page=${pageNo}&sort_by=popularity.desc&with_genres=10751&year=2020`   );
              const data1 = await kids.json();
      ...
      dispatch(setApiData({kids:combinedData}));
    } catch (error) {
      console.error(error);
    }
  };
