...
useEffect(() => {
  const fetchDataFromAPI = async () => {
    try {
      const documentory = await fetch( `https://api.themoviedb.org/3/discover/movie?api_key=cfe422613b250f702980a3bbf9e90716&page=${pageNo}&sort_by=vote_count.desc&with_genres=99&year=2004`);
              const data1 = await documentory.json();
   ...
   dispatch(setApiData({documentry:combinedData}));
    } catch (error) {
      console.error(error);
    }
  };
