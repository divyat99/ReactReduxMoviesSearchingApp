...
const Music = () => {
    const music=useSelector((state)=>state.searchbar.apiData.music)
 ...
 const [pageNo, setPageNo] = useState(2);
  ...
  useEffect(() => {
    const fetchDataFromAPI = async () => {
      try {
        const music= await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=cfe422613b250f702980a3bbf9e90716&page=${pageNo}&sort_by=popularity.desc&with_genres=10402&year=2022`);
                const data1 = await music.json();
          ...
          dispatch(setApiData({music:combinedData}));
