...
useEffect(() => {
  const fetchDataFromAPI = async () => {
    try {
      const adventure = await fetch(`https:api.themoviedb.org/3/discover/movie?api_key=cfe422613b250f702980a3bbf9e90716&page=${pageNo}&sort_by=popularity.desc&with_genres=12&year=1986`
             );
              const data1 = await adventure.json();
  ...
  dispatch(setApiData({adventure:combinedData}));
    } catch (error) {
      console.error(error);
    }
  };
